...
if __name__ == "__main__":
    ...
    model_name = "models--lxyuan--distilbert-base-multilingual-cased-sentiments-student"
    ...
    model = DistilBertForSequenceClassification.from_pretrained(model_name, num_labels=num_labels, id2label=id_to_label,
                                                                label2id=label_to_id, ignore_mismatched_sizes=True)   # 加上这一行！
    # 忽略分类头的维度不匹配
    # 如果你确实只想训练一个二分类模型（即 num_labels=2），
    # 只需要在 from_pretrained() 时加上参数：
    # ignore_mismatched_sizes=True

    # ✅ 这样会跳过原分类层权重加载，重新初始化新的分类层（随机初始化）。
